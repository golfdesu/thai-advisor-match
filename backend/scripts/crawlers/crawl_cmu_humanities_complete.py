# -*- coding: utf-8 -*-
"""
Autonomous 5-Pillar Data Acquisition: CMU Faculty of Humanities
Target: คณะมนุษยศาสตร์ มหาวิทยาลัยเชียงใหม่ (All 10 Departments)
...
DEPARTMENTS = [
    ("3798e6ff-5704-4a4d-b61a-70f9f3f97229", "ภาควิชาการท่องเที่ยว"),
    ("c4f65d38-c033-43be-b837-8da56b41cdb7", "ภาควิชาจิตวิทยา"),
    ("10c10766-c436-4f19-b62b-d22c0f076de3", "ภาควิชาบรรณารักษศาสตร์และสารสนเทศศาสตร์"),
    ("9ad3e298-c0ea-4032-9088-fff94122be3a", "ภาควิชาประวัติศาสตร์"),
    ("f7371201-14cf-4d46-b4ed-de2b00ecc141", "ภาควิชาปรัชญาและศาสนา"),
    ("5920b963-721a-44bc-a3e0-388fef8828d3", "ภาควิชาภาษาตะวันตก"),
    ("0fa1faa1-3afe-4da9-81db-d2d4abaefb2f", "ภาควิชาภาษาตะวันออก"),
    ("5d9ec8c5-f8ee-4de8-9d87-c06b2bc73a84", "ภาควิชาภาษาไทย"),
    ("db5d90f4-2c56-4af1-9237-cb57411131ca", "ภาควิชาภาษาอังกฤษ"),
    ("931f7c48-519e-457a-beaa-5524021e1a9b", "ภาควิชามนุษยสัมพันธ์")
]
...
def clean_en_name(raw_en):
    ...
    # strip trailing titles e.g. ", Lecturer", ", Ph.D."
...
def fetch_department_faculty(dept_uuid, dept_name):
    th_url = f"https://www.cmu.ac.th/th/faculty/humanities/teacher/{dept_uuid}"
    en_url = f"https://www.cmu.ac.th/en/faculty/humanities/teacher/{dept_uuid}"
    ...
    try:
        ...
        print(f"Error fetching TH for {dept_name}: {e}")
    ...
    try:
        ...
        print(f"Error fetching EN for {dept_name}: {e}")
    ...
    for email, th_info in th_records.items():
        ...
        # If Thai name is foreign characters, adjust
...
def main():
    print("=" * 60)
    print("  WAVE 88: CMU HUMANITIES AUTONOMOUS ACQUISITION PIPELINE")
    print("=" * 60)

    # Phase 1: Harvesting all 10 departments
    print("\nPhase 1: Harvesting 10 departmental directories...")
    all_faculties = []
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [executor.submit(fetch_department_faculty, uid, dname) for uid, dname in DEPARTMENTS]
        for f in futures:
            res = f.result()
            all_faculties.extend(res)

    print(f"Total faculty records harvested: {len(all_faculties)}")
    ...
    for fac in all_faculties:
        ...
        if em not in unique_faculties:
            ...
            # retain the record with image_url if current doesn't have
    ...
    chk_path = out_dir / "wave88_cmu_humanities_extraction.json"
    ...
    cur.execute("""
        ...
        FROM faculties
        WHERE university_th = 'มหาวิทยาลัยเชียงใหม่' AND faculty_th = 'คณะมนุษยศาสตร์'
    """)
    existing_db = cur.fetchall()
    print(f"Existing CMU Humanities records in DB: {len(existing_db)}")
    ...
    for fac in records:
        ...
        if row:
            ...
            if row2:
                ...
                # Insert fresh record
                import uuid
                new_id = f"cmu_hum_{uuid.uuid4().hex[:10]}"
                cur.execute("""
                    INSERT INTO faculties (
                        ...
                        %s, 'Chiang Mai University', 'มหาวิทยาลัยเชียงใหม่', 'Faculty of Humanities', 'คณะมนุษยศาสตร์', %s,
                        %s, %s,
                        %s, %s, %s, %s, %s,
                        %s, %s, %s, %s, %s::json
                    )
