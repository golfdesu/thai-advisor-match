"""Wave25 ingest: 2 committable exports (KAPI, KMUTNB indtech) + strict filters."""
...
EXPECT = {
 "wave25_ku_kapi_export.py": "เกษตร",
 "wave25_kmutnb_indtech_export.py": "พระนครเหนือ",
}
...
def real_person(s):
    ...
    if re.search(r'[A-Za-z]{4,}\s+[A-Za-z]{4,}', s) and not re.search(r'[ก-๙]', s): return False
...
def main():
    ...
    try:
        ...
        _log(f"existing in scope: {len(existing)}")
        ...
        if new_members:
            ...
            for m, v in zip(new_members, vecs):
                n += 1
                uid = f"wave25_{n:04d}_{random.randint(100,999)}"
                while uid in have:
                    n += 1; uid = f"wave25_{n:04d}_{random.randint(100,999)}"
                have.add(uid)
