"""Wave35 ingest: MJU erp x3 + MFU liberalarts/antiaging + MSU finearts + NIDA gseda."""
...
EXPECT = {
 "wave35_mju_fishery_export.py": ("แม่โจ้", {}),
 "wave35_mju_animal_export.py": ("แม่โจ้", {}),
 "wave35_mju_energy_export.py": ("แม่โจ้", {}),
 "wave35_msu_finearts_export.py": ("มหาสารคาม", {}),
 "wave35_mfu_liberalarts_export.py": ("แม่ฟ้าหลวง", {}),
 "wave35_mfu_antiaging_export.py": ("แม่ฟ้าหลวง", {"dept_contains": "ชะลอวัย"}),
 "wave35_nida_gseda_export.py": ("บัณฑิตพัฒนบริหารศาสตร์", {}),
}
# faculty_th override per file (consolidate renamed units to grad-mapped label)
FACULTY_OVERRIDE = {
 "wave33_nu_sgtech_export.py": "วิทยาลัยพลังงานทดแทนและสมาร์ตกริดเทคโนโลยี (SGtech)",
}
CANON = {}
...
def main():
    ...
    for fname, (uni_key, opts) in EXPECT.items():
        ...
        for r in rows:
            ...
            dc = opts.get("dept_contains")
            if dc and dc not in (r.get("department_th") or ""):
                xdept += 1; continue
            if "บัณฑิตพัฒนบริหารศาสตร์" in (r.get("university_th") or ""):
                r["university_th"] = "สถาบันบัณฑิตพัฒนบริหารศาสตร์ (นิด้า)"
            ...
            if opts.get("scrub_lists"):
                ...
                r["taught_courses"] = []
            if fname in FACULTY_OVERRIDE:
                r["faculty_th"] = FACULTY_OVERRIDE[fname]
            if "บัณฑิตพัฒนบริหารศาสตร์" in (r.get("university_th") or ""):
                r["university_th"] = "สถาบันบัณฑิตพัฒนบริหารศาสตร์ (นิด้า)"
    ...
    try:
        ...
        if new_members:
            ...
            for m, v in zip(new_members, vecs):
                n += 1
                uid = f"wave30_{n:04d}_{random.randint(100,999)}"
                while uid in have:
                    n += 1; uid = f"wave30_{n:04d}_{random.randint(100,999)}"
                have.add(uid)
