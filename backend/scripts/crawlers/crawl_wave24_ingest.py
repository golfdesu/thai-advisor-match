"""Wave24 ingest: university-enforced + sanitize + grouped dedup + vectorize + local commit."""
...
EXPECT = {
 "wave24_cu_spsc_export.py": "จุฬาลงกรณ์",
 "wave24_su_archaeo_export.py": "ศิลปากร",
 "wave24_su_ict_export.py": "ศิลปากร",
 "wave24_su_ict_b_export.py": "ศิลปากร",
 "wave24_su_ms_export.py": "ศิลปากร",
 "wave24_swu_cosci_export.py": "ศรีนครินทรวิโรฒ",
 "wave24_swu_pt_export.py": "ศรีนครินทรวิโรฒ",
 "wave24_tu_litu_export.py": "ธรรมศาสตร์",
 "wave24_tu_ssd_export.py": "ธรรมศาสตร์",
 "wave24_tu_tds_export.py": "ธรรมศาสตร์",
 "wave24_mfu_law_export.py": "แม่ฟ้าหลวง",
 "wave24_ubu_sci_deep_export.py": "อุบล",
 "wave24_up_agri_export.py": "พะเยา",
 "wave24_up_libarts_export.py": "พะเยา",
}
...
def main():
    ...
    for fname, uni_key in EXPECT.items():
        ...
        _log(f"{fname}: raw {len(rows)} kept {kept} xuni {xuni} xname {xname}")
    ...
    try:
        ...
        if new_members:
            ...
            for m, v in zip(new_members, vecs):
                n += 1
                uid = f"wave24_{n:04d}_{random.randint(100,999)}"
                while uid in have:
                    n += 1; uid = f"wave24_{n:04d}_{random.randint(100,999)}"
                have.add(uid)
