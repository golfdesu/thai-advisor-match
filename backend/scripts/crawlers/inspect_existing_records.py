...
cur.execute("""
    SELECT id, full_name_th, first_name, last_name, academic_title_th, department_th, email, openalex_id
    FROM faculties
    WHERE university_th = 'มหาวิทยาลัยสงขลานครินทร์' AND faculty_th ILIKE '%ทันต%';
""")
rows = cur.fetchall()
print(f"PSU Dent in DB ({len(rows)}):")
for r in rows:
    print(" ", r)
...
""")
rows2 = cur.fetchall()
print(f"\nSWU Humanities in DB ({len(rows2)}):")
for r in rows2:
    print(" ", r)
