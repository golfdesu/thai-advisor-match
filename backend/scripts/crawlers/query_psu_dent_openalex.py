...
url = "https://api.openalex.org/authors?filter=last_known_institutions.id:I131868736&per-page=5"
req = urllib.request.Request(url, headers={"User-Agent": "ThaiEduCenter/1.0 (mailto:admin@thaieducenter.org)"})
with urllib.request.urlopen(req) as resp:
    data = json.loads(resp.read().decode())
    print(f"PSU total authors in OpenAlex: {data.get('meta', {}).get('count')}")
