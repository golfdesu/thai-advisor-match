"""Wave28 MFU-A ingest: law/sinology/management. MFU-scoped dedup + vectorize + commit."""
...
EXPECT = {
 "wave28_mfu_sinology_export.py": "แม่ฟ้าหลวง",
 "wave28_mfu_law_export.py": "แม่ฟ้าหลวง",
 "wave28_mfu_management_export.py": "แม่ฟ้าหลวง",
 "wave28_mfu_science_export.py": "แม่ฟ้าหลวง",
}
...
def main():
    ...
    try:
        ...
        _log(f"existing MFU in DB: {len(existing)}")
        clean_cache = {}
        def clean_of(ex):
            v = clean_cache.get(ex.id)
            if v is None: v = strip_all_titles(ex.full_name_th or ""); clean_cache[ex.id] = v
            return v
        for ex in existing: clean_of(ex)
        ...
        for m in cands:
            ...
            match = None
            if existing:
                choices = {ex.id: clean_of(ex) for ex in existing}
                hit = process.extractOne(mc, choices, scorer=fuzz.token_set_ratio, score_cutoff=90)
                if hit: match = next((e for e in existing if e.id == hit[2]), None)
            if match:
                ch = False
        ...
        if new_members:
            ...
            for m, v in zip(new_members, vecs):
                n += 1
                uid = f"wave28_{n:04d}_{random.randint(100,999)}"
                while uid in have:
                    n += 1; uid = f"wave28_{n:04d}_{random.randint(100,999)}"
                have.add(uid)
