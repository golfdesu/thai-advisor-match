"""Wave 59: Low-OPX Expansion + CRU Sub-Institutions

Targets universities with low OpenAlex roster coverage:
- TSU  (Thaksin):          1,881 total, 25% OPX →  1,334 OPX authors available
- WU   (Walailak):         2,334 total, 52% OPX →  3,397 OPX authors available
- UP   (Phayao):           1,261 total, 68% OPX →  2,499 OPX authors available
- UBU  (Ubon Ratchathani): 941 total,  71% OPX →  2,006 OPX authors available
- BUU  (Burapha):          4,443 total, 82% OPX →  3,663 OPX authors available

Plus CRU sub-institutions (umbrella ID had 0 authors in wave 58):
- CRU_RI (Chulabhorn Research Institute):  935 authors
- CRU_GI (Chulabhorn Graduate Institute):  609 authors
- CRU_H  (Chulabhorn Hospital):          1,296 authors

All IDs verified via OpenAlex institution search before script creation.
"""
...
# ─────────────────────────────────────────────────────────────
# Target Universities — all IDs verified via OpenAlex search
# ─────────────────────────────────────────────────────────────
TARGET_UNIVERSITIES = [
    # Priority 1: Very low OPX coverage
    {"en": "Thaksin University",                      "th": "มหาวิทยาลัยทักษิณ",
     "prefix": "tsu",    "openalex_id": "I79246082",   "current_total": 1881, "current_opx_pct": 25},
    {"en": "Walailak University",                     "th": "มหาวิทยาลัยวลัยลักษณ์",
     "prefix": "wu",     "openalex_id": "I96916377",   "current_total": 2334, "current_opx_pct": 52},
    {"en": "University of Phayao",                    "th": "มหาวิทยาลัยพะเยา",
     "prefix": "up",     "openalex_id": "I4210090662", "current_total": 1261, "current_opx_pct": 68},
    {"en": "Ubon Ratchathani University",             "th": "มหาวิทยาลัยอุบลราชธานี",
     "prefix": "ubu",    "openalex_id": "I72091625",   "current_total": 941,  "current_opx_pct": 71},
    {"en": "Burapha University",                      "th": "มหาวิทยาลัยบูรพา",
     "prefix": "buu",    "openalex_id": "I129488602",  "current_total": 4443, "current_opx_pct": 82},

    # Priority 2: CRU sub-institutions (umbrella I4405255716 = 0 authors)
...
RE_BAD_NAME = re.compile(
    r"(?:ภาควิชา|สาขาวิชา|หน่วยงาน|คณะ|department|faculty|center|school|hospital)", re.I
)
...
def process_university(univ: dict, db, have_ids_global: set) -> tuple[int, int]:
    ...
    # Checkpoint
    ckpt = CHECKPOINT_DIR / f"wave59_{univ['prefix']}_extraction.json"
    ...
    # Commit in batches of 300
    new_objs = []
    seq = 0
    for i, r in enumerate(to_insert):
        seq += 1
        uid = f"{univ['prefix']}_w59_{seq:04d}_{random.randint(100, 999)}"
...
def main():
    print("=" * 70)
    print("Wave 59: Low-OPX Expansion + CRU Sub-Institutions Pipeline")
    print(f"         {len(TARGET_UNIVERSITIES)} targets")
    print("=" * 70)
    ...
    try:
        ...
        for idx, univ in enumerate(TARGET_UNIVERSITIES):
            ...
            if idx < len(TARGET_UNIVERSITIES) - 1:
                if inserted == 0 and enriched == 0:
                    print(f"  [cooldown] No data — waiting 90s...")
                    time.sleep(90)
                else:
                    time.sleep(10)
        ...
        print(f"  Wave 59: +{total_inserted:,} new, {total_enriched:,} enriched")

    finally:
        db.close()

    print("\nWave 59 Pipeline Complete!")
