"""Wave36b: ingest EN-format real names wrongly rejected by Thai-script rule."""
...
RE_DEGSUF = re.compile(r",?\s*(Ph\.?\s?D\.?|M\.?\s?A\.?|M\.?\s?S\.?c\.?|Ed\.?\s?D\.?|D\.?\s?Phil\.?)\.?$", re.I)
RE_TH_TITLE = re.compile(r"^(อ\.|ผศ\.|รศ\.|ศ\.|ดร\.|ผศ\.ดร\.|รศ\.ดร\.|ศ\.ดร\.|อ\.ดร\.|Mr\.|Mrs\.|Ms\.|Dr\.)\s+", re.I)
RE_ENWORD = re.compile(r"^[A-Z][a-zA-Z'\-éèêëáàâäíìîïóòôöúùûüñçøåæœ]{1,}$")

FILES = {
 "wave36_kku_huso_export.py": "ขอนแก่น",
 "wave36_mfu_adt_export.py": "แม่ฟ้าหลวง",
 "wave36_su_ict_export.py": "ศิลปากร",
}

def clean_name(s):
    s = (s or "").strip()
    s = RE_DEGSUF.sub("", s).strip().rstrip(",").strip()
    return s

def real_person_plus(s):
    s = clean_name(s)
    ...
    try:
        ...
        base = ""
    if base and len(base) >= 4 and len(base) <= 60 and len(base.split()) >= 2 \
            and re.search(r'[ก-๙]{2,}', base):
        return True
    # EN-name path: Thai/EN academic title + 2+ capitalized tokens
    if not RE_TH_TITLE.match(s): return False
    rest = RE_TH_TITLE.sub("", s).strip()
    toks = rest.split()
    if len(toks) < 2: return False
    good = [t for t in toks if RE_ENWORD.match(t)]
    if len(good) < 2: return False
    if len(rest) > 60: return False
    return True
...
def main():
    ...
    cands = []
    for fname, uni_key in FILES.items():
        rows = load_export(d / fname)
        kept = 0
        for r in rows:
            u = (r.get("university_th") or "") + (r.get("university") or "")
            if uni_key not in u: continue
            if not real_person_plus(r.get("full_name_th", "")): continue
            # skip already-committed Thai-script ones: dedup below handles it
            r["full_name_th"] = clean_name(r.get("full_name_th", ""))
            r["email"] = clean_email(r.get("email"))
            # split EN first/last for EN names lacking them
            if not re.search(r'[ก-๙]', r.get("first_name") or ""):
                rest = RE_TH_TITLE.sub("", r["full_name_th"]).strip()
                parts = rest.split()
                if len(parts) >= 2 and not (r.get("first_name") or "").strip():
                    r["first_name"] = parts[0]
                    r["last_name"] = " ".join(parts[1:])
            cands.append(r); kept += 1
        _log(f"{fname}: raw {len(rows)} pass-plus {kept}")
    _log(f"TOTAL pass-plus: {len(cands)}")
    db = SessionLocal()
    try:
        ...
        for m in cands:
            mc = strip_all_titles((m.get("full_name_th") or "").strip())
        ...
        _log(f"net new EN-names: {len(new_members)}")
        ...
        if new_members:
            ...
            for m, v in zip(new_members, vecs):
                n += 1
                uid = f"wave36b_{n:04d}_{random.randint(100,999)}"
                while uid in have:
                    n += 1; uid = f"wave36b_{n:04d}_{random.randint(100,999)}"
                have.add(uid)
