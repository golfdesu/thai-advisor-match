"""Wave27 ingest: MU dept breakthrough (rilca/mb/ipsr) + NIDA env + re-yield dedup."""
...
EXPECT = {
 "wave27_mu_rilca_export.py": "มหิดล",
 "wave27_mu_mb_export.py": "มหิดล",
 "wave27_mu_ipsr_export.py": "มหิดล",
 "wave27_nida_env_export.py": "บัณฑิตพัฒนบริหารศาสตร์",
 "wave27_nida_bus_export.py": "บัณฑิตพัฒนบริหารศาสตร์",
 "wave27_su_ict_export.py": "ศิลปากร",
}
    ...
    try:
        ...
        if new_members:
            ...
            for m, v in zip(new_members, vecs):
                n += 1
                uid = f"wave27_{n:04d}_{random.randint(100,999)}"
                while uid in have:
                    n += 1; uid = f"wave27_{n:04d}_{random.randint(100,999)}"
                have.add(uid)
