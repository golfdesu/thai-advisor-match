"""Apply Phase 24 recovered authentic official emails to local PostgreSQL.

Phase 24:
1. Ingests 280 verified authentic university emails:
...
2. Confirms that unresolvable entries in CMU Medicine Surgery, PSU Medicine, KMITL Architecture,
   Chula Math, and retired emeritus faculty remain SQL NULL under Section 9 Quality Invariants and PDPA
...
DATA_FILE = BACKEND_DIR / "data" / "agent_states" / "recoverable_official_emails_phase24.json"
...
def main(*, apply: bool) -> None:
    ...
    try:
        ...
        out_filename = (
            "recovered_official_emails_phase24_apply.json"
            if apply
            else "recovered_official_emails_phase24_dryrun.json"
        )
        ...
        if apply:
            db.commit()
            print("Successfully committed all Phase 24 mutations to local PostgreSQL.")
        else:
            db.rollback()
