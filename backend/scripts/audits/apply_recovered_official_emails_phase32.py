"""Apply Phase 32 multi-agent recovered authentic official emails to local PostgreSQL.

Phase 32 Target High-Yield Clusters:
- Ubon Ratchathani University - Faculty of Pharmacy (phar.ubu.ac.th) (37 records)
- Chulalongkorn University - Faculty of Science (sc.chula.ac.th) (3 records: Chem, MatSci, Geo)

Strict Section 9 Invariant Compliance:
- Rejects personal freemails (@gmail, @hotmail, @yahoo, etc.)
- Rejects generic inboxes (info@, contact@, saraban@, phar@, chemistry@, etc.)
...
DEFAULT_INPUT_FILE = DATA_DIR / "recoverable_official_emails_phase32.json"
...
def main(apply: bool = False, input_file: str | None = None) -> None:
    ...
    try:
        ...
        out_filename = (
            "recovered_official_emails_phase32_apply.json"
            if apply
            else "recovered_official_emails_phase32_dryrun.json"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Apply Phase 32 recovered official emails to PostgreSQL.")
