"""Apply Phase 33 multi-agent recovered authentic official emails to local PostgreSQL.

Phase 33 Target High-Yield Clusters:
- Thammasat University - Faculty of Allied Health Sciences (allied.tu.ac.th) (4 records)
- Kasetsart University - Faculty of Science (sci.ku.ac.th) (7 records: Chem, Micro, MatSci, CS, Zoo)
- Mahidol University - College of Music (music.mahidol.ac.th) (1 record)

Strict Section 9 Invariant Compliance:
- Rejects personal freemails (@gmail, @hotmail, @yahoo, @outlook, etc.)
- Rejects generic inboxes (info@, contact@, saraban@, etc.)
- Validates institutional domains (.ac.th, .edu, ku.th, etc.)
...
DEFAULT_INPUT_FILE = DATA_DIR / "recoverable_official_emails_phase33.json"
...
REJECT_GENERIC = {
    ...
    "fibo@", "fin@", "help@", "service@", "press@", "agriubu@", "la@",
    "phar@", "phar_it@", "chemistry@", "sci@", "sciest@", "ma.sci@",
    "zoo.sci@", "cheminfo@"
}
VALID_SUFFIXES = (
...
def main(apply: bool = False, input_file: str | None = None) -> None:
    ...
    try:
        ...
        out_filename = (
            "recovered_official_emails_phase33_apply.json"
            if apply
            else "recovered_official_emails_phase33_dryrun.json"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Apply Phase 33 recovered official emails to PostgreSQL.")
