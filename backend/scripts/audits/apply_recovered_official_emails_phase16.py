...
Phase 16:
- Ingests 244 verified authentic university emails recovered from 1-to-1 official faculty profiles:
  * CMU Dentistry (94 faculty members via dent.cmu.ac.th /web/staff/<email>)
  * PSU Agro-Industry (44 faculty members via agro.psu.ac.th /agro6/staff/)
  * KU Chemistry (41 faculty members via chemy.sci.ku.ac.th /ku-personnel/<slug>/)
  * CMU Mathematics (36 faculty members via math.science.cmu.ac.th personals-detail.php?id=...)
  * KU Forest Deep Retry (29 faculty members via Person.aspx?id=...)
- Excludes all departmental shared inboxes and existing collisions.
- Also updates profile_url for CMU Dent, CMU Math, and KU Chem to their individual 1-to-1 profile URLs.
...
DATA_FILE = BACKEND_DIR / "data" / "agent_states" / "recoverable_official_emails_phase16.json"
        ...
        for item in valid_items:
            ...
            if changed:
                ...
                report["updated_faculties"].append({
                    ...
                    "new_email": new_email,
                    "new_profile_url": f.profile_url,
                })

        report["embedding_texts_rebuilt"] = rebuilt_count

        out_filename = (
            "recovered_official_emails_phase16_apply.json"
            if apply
            else "recovered_official_emails_phase16_dryrun.json"
        )
