...
def clean_title(name: str | None) -> str:
    if not name:
        return ''
    res = re.sub(r'\s+', ' ', name).strip()
    for _ in range(3):
        res = re.sub(
            r'^(ศาสตราจารย์\s+ดร\.|รองศาสตราจารย์\s+ดร\.|ผู้ช่วยศาสตราจารย์\s+ดร\.|อาจารย์\s+ดร\.|'
            r'ศาสตราจารย์|รองศาสตราจารย์|ผู้ช่วยศาสตราจารย์|อาจารย์|'
            r'ศ\.ดร\.|รศ\.ดร\.|ผศ\.ดร\.|อ\.ดร\.|ดร\.|ศ\.|รศ\.|ผศ\.|อ\.|'
            r'Prof\.\s+Dr\.|Assoc\.\s+Prof\.\s+Dr\.|Asst\.\s+Prof\.\s+Dr\.|Prof\.|Assoc\.\s+Prof\.|Asst\.\s+Prof\.|Dr\.)\s*',
            '',
            res,
            flags=re.IGNORECASE
        ).strip()
