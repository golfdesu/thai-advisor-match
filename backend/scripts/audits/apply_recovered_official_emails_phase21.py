"""Apply Phase 21 recovered authentic official emails to local PostgreSQL.

Phase 21:
1. Ingests 175 verified authentic university emails:
   - Mahidol University College of Music (118 faculty members via @mahidol.ac.th, @mahidol.edu)
   - Thammasat University Faculty of Medicine (31 faculty members via @tu.ac.th)
   - Chulalongkorn University Sasin School of Management (26 faculty members via @sasin.edu)
2. Confirms that all remaining faculty records in audited clusters (CBS, Med CMU, Dent Chula, KMITL Arch, KU Agr, PSU Med, Silpakorn Eng) remain SQL NULL under Section 9 Invariant 10 & PDPA.
...
DATA_FILE = BACKEND_DIR / "data" / "agent_states" / "recoverable_official_emails_phase21.json"
    ...
    try:
        ...
        for item in valid_items:
            ...
            if new_source and any(k in new_source for k in [
                "music.mahidol.ac.th/people/",
                "sasin.edu/team/profile/",
                "med.tu.ac.th/department/attm/",
                "med.tu.ac.th/cmfm/"
            ]):
        ...
        out_filename = (
            "recovered_official_emails_phase21_apply.json"
            if apply
            else "recovered_official_emails_phase21_dryrun.json"
        )
        ...
        if apply:
            db.commit()
            print("Successfully committed all Phase 21 mutations to local PostgreSQL.")
        else:
            db.rollback()
