"""Apply Phase 31 multi-agent recovered authentic official emails to local PostgreSQL.

Phase 31 Target High-Yield Clusters:
- Silpakorn Materials Science & Engineering (matse.su.ac.th)
- Ubon Ratchathani University - Faculty of Agriculture (agri.ubu.ac.th)
- Ubon Ratchathani University - Faculty of Liberal Arts (la.ubu.ac.th)

Strict Section 9 Invariant Compliance:
- Rejects personal freemails (@gmail, @hotmail, @yahoo, etc.)
- Rejects generic inboxes (info@, contact@, saraban@, agriubu@, la@, etc.)
...
DEFAULT_INPUT_FILE = DATA_DIR / "recoverable_official_emails_phase31.json"
...
def main(apply: bool = False, input_file: str | None = None) -> None:
    ...
    try:
        ...
        out_filename = (
            "recovered_official_emails_phase31_apply.json"
            if apply
            else "recovered_official_emails_phase31_dryrun.json"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Apply Phase 31 recovered official emails to PostgreSQL.")
