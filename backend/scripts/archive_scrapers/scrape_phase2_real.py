...
TARGETS=[
    ("National Institute of Development Administration","https://nida.ac.th/en/program/master/", "NIDA master"),
    ("National Institute of Development Administration","https://nida.ac.th/en/program/phd/", "NIDA PhD"),
    ("Chulalongkorn University","https://www.chula.ac.th/en/academics/programs/", "Chula programs"),
    ("Chulalongkorn University","https://academic.chula.ac.th/detail/news/49", "Chula summary"),
    ("Mahidol University","https://graduate.mahidol.ac.th/inter/prospective-students/?p=curriculum", "MU grad"),
    ("Chiang Mai University","https://mis.cmu.ac.th/TQF/TQF2/CurriculumPublicList.aspx", "CMU TQF"),
]
...
def extract(txt, url, uni):
    if len(txt)<500:
        print(f" skip short {url[:60]} {len(txt)}")
        return []
...
"""
    ...
    try:
        ...
        if resp.text:
            ...
            for c in courses:
                c["website_url"]=url
            print(f"  -> {len(courses)} courses from {url[:60]}")
            return courses
    except Exception as e:
...
def main():
    ...
    all_courses=[]
    for uni, url, label in TARGETS:
        print(f"--- {label} {uni} --- {url}")
        txt=fetch(url)
    ...
    with engine.connect() as conn:
        for uni in ["National Institute of Development Administration","Chulalongkorn University","Mahidol University","Chiang Mai University"]:
