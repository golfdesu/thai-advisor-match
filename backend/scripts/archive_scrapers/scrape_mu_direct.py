...
TARGETS=[
    ...
    ("Mahidol University","https://mathematics.sc.mahidol.ac.th/th/programs-th/"),
    ("Mahidol University","https://science.mahidol.ac.th/graduate_programs.php"),
    ("Mahidol University","https://www.si.mahidol.ac.th/th/department/"),
]
...
def main():
    ...
    for uni, url in TARGETS:
        ...
        time.sleep(3)  # avoid 20/min quota
    ...
    with engine.connect() as conn:
        mu_cnt=conn.execute(text("SELECT count(*) FROM courses WHERE university='Mahidol University'")).scalar()
        print(f"MU total {mu_cnt}")
        total=conn.execute(text('SELECT count(*) FROM courses')).scalar()
        print(f"Total {total}")
