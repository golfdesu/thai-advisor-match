...
logger = logging.getLogger("Advisor_Agent2")

from google import genai
from google.genai import types

UNIVERSITIES = ["Mahidol University"]
FACULTIES = [
...
def run_scraper():
    ...
    with Session(engine) as session:
        for uni in UNIVERSITIES:
            for fac in FACULTIES:
                logger.info(f"=== Advisor Agent 2 Target: {uni} | {fac} ===")
    ...
    logger.info(f"Advisor Agent 2 completed! Total: {total_scraped}")
