# -*- coding: utf-8 -*-
"""
Resolve Wave 74 Duplicates & Grounding Merge
============================================
Merges duplicates and consolidates profiles following Wave 74 acquisition:
1. Harmonizing Mahidol Human Rights & Child Development Courses:
   - 'โครงการจัดตั้งสถาบันสิทธิมนุษยชนและสันติศึกษา' -> 'สถาบันสิทธิมนุษยชนและสันติศึกษา'
   - 'โครงการร่วมคณะแพทยศาสตร์โรงพยาบาลรามาธิบดี คณะแพทยศาสตร์ศิริราชพยาบาล สถาบันแห่งชาติเพื่อการพัฒนาเด็กและครอบครัว'
     -> 'สถาบันแห่งชาติเพื่อการพัฒนาเด็กและครอบครัว'
2. Consolidating intra-university duplicate listings (e.g. Weerawat Limroongreungrat, Kornkit Chaijenkij).
3. Merging new graduate-level records while preserving highest citations, images, and official emails.
4. Archiving unassigned/non-teaching placeholder records (e.g. mu_w57_1004_488) to public.scholars_unassigned.
...
def resolve_wave74_duplicates():
    print("=================================================================", flush=True)
    print("🔄 RESOLVING WAVE 74 DUPLICATES & PROFILE MERGING", flush=True)
    print("=================================================================", flush=True)

    db = SessionLocal()
    try:
        # Step 0: Harmonize Course-to-Faculty Naming in courses & faculties
        print("\n--- Harmonizing Mahidol Graduate Courses Naming ---", flush=True)
        course_mappings = [
            ("มหาวิทยาลัยมหิดล", "โครงการจัดตั้งสถาบันสิทธิมนุษยชนและสันติศึกษา", "สถาบันสิทธิมนุษยชนและสันติศึกษา"),
            ("มหาวิทยาลัยมหิดล", "โครงการร่วมคณะแพทยศาสตร์โรงพยาบาลรามาธิบดี คณะแพทยศาสตร์ศิริราชพยาบาล สถาบันแห่งชาติเพื่อการพัฒนาเด็กและครอบครัว", "สถาบันแห่งชาติเพื่อการพัฒนาเด็กและครอบครัว"),
        ]
        ...
        # Step 2: Clean anomalous entries, fix slashes in interests, and publication shape for Wave 74
        wave74_prefixes = ["mu_ihrp%", "mu_sports%", "mu_nicfd%", "mu_la%"]
        conditions = [FacultyDB.id.like(p) for p in wave74_prefixes]
        from sqlalchemy import or_
        all_wave_records = db.query(FacultyDB).filter(or_(*conditions)).all()

        for f in all_wave_records:
            # Fix double title prefixes
            for prefix in ["ศ.เกียรติคุณ นพ.", "ศ.เกียรติคุณ", "ศ.ดร.", "รศ.ดร.", "ผศ.ดร.", "อ.ดร.", "ศ.", "รศ.", "ผศ.", "อ.", "ดร.", "พญ.", "นพ."]:
                dbl = f"{prefix} {prefix}"
            ...
            # Deduplicate research interests, split " / ", and strip punctuation
        ...
        # Step 2b: Intra-University & Historical Seed Consolidations
        print("\n--- Intra-University Seed Consolidations ---", flush=True)

        # 1. Weerawat Limroongreungrat: merge cu_w58_1854_778 into active mu_sports__004
        p_wee = db.query(FacultyDB).filter(FacultyDB.id == "mu_sports__004").first()
        s_wee = db.query(FacultyDB).filter(FacultyDB.id == "cu_w58_1854_778").first()
        if p_wee and s_wee:
            print("  [Merge] Merging cu_w58_1854_778 into active mu_sports__004 (Weerawat Limroongreungrat)")
            merge_faculty_pair(p_wee, s_wee, db)
            p_wee.openalex_id = "A5021134597"
            p_wee.total_citations = max(p_wee.total_citations or 0, 752)
            p_wee.h_index = max(p_wee.h_index or 0, 15)
            p_wee.total_publications_count = max(p_wee.total_publications_count or 0, 112)

        # 2. Kornkit Chaijenkij: merge mu_sports__006 into mu_sports_kornkit_001
        p_korn = db.query(FacultyDB).filter(FacultyDB.id == "mu_sports_kornkit_001").first()
        s_korn = db.query(FacultyDB).filter(FacultyDB.id == "mu_sports__006").first()
        if p_korn and s_korn:
            print("  [Merge] Merging mu_sports__006 into active mu_sports_kornkit_001 (Kornkit Chaijenkij)")
            merge_faculty_pair(p_korn, s_korn, db)

        # 3. Sriprapha Petcharamesree: merge former Mahidol IHRP record into active Chulalongkorn Law faculty
        p_sri = db.query(FacultyDB).filter(FacultyDB.id == "chulalongk_facultyofl_petcharatana_045").first()
        s_sri = db.query(FacultyDB).filter(FacultyDB.id == "mu_ihrp__009").first()
        if p_sri and s_sri:
            print("  [Transfer Merge] Merging Mahidol IHRP record (mu_ihrp__009) into active CU Law faculty (chulalongk_facultyofl_petcharatana_045)")
            merge_faculty_pair(p_sri, s_sri, db)
            p_sri.openalex_id = "A5054459119"
            p_sri.total_citations = max(p_sri.total_citations or 0, 148)
            p_sri.h_index = max(p_sri.h_index or 0, 5)
            p_sri.total_publications_count = max(p_sri.total_publications_count or 0, 14)

        # 4. Archive unassigned blank placeholder mu_w57_1004_488 to scholars_unassigned
        blank_rec = db.query(FacultyDB).filter(FacultyDB.id == "mu_w57_1004_488").first()
        if blank_rec:
            print("  [Archive] Moving unassigned scholar placeholder mu_w57_1004_488 to scholars_unassigned")
            with engine.begin() as conn:
                conn.execute(
                    text("""
                        INSERT INTO public.scholars_unassigned SELECT * FROM public.faculties WHERE id = :sid
                ...
                conn.execute(text("DELETE FROM public.faculties WHERE id = :sid"), {"sid": "mu_w57_1004_488"})
        ...
        # Email hygiene & trailing junk removal for Wave 74
        for f in all_wave_records:
            if f.email:
    ...
    print("\n🎉 Wave 74 duplicate resolution completed successfully!")


if __name__ == "__main__":
    resolve_wave74_duplicates()
