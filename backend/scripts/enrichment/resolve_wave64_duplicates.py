# -*- coding: utf-8 -*-
"""
Resolve Wave 64 Duplicates & Grounding Merge
============================================
Merges duplicates and consolidates profiles following Wave 64 acquisition:
1. Intra-university duplicate listings (e.g. KKU Vet, KU FLAS, KMUTNB FTE).
2. Normalizing CMMU and RILCA faculty_th for 100% curriculum linkage.
...
7. Archiving merged secondary records to public.scholars_unassigned.
"""
...
def resolve_wave64_duplicates():
    print("=================================================================", flush=True)
    print("🔄 RESOLVING WAVE 64 DUPLICATES & PROFILE MERGING", flush=True)
    print("=================================================================", flush=True)

    db = SessionLocal()
    try:
        # Step 0: Synchronize Mahidol graduate unit names for 100% curriculum linkage
        print("\n--- Synchronizing Mahidol Graduate Unit Names ---", flush=True)
        cmmu_count = (
            db.query(FacultyDB)
            .filter(FacultyDB.university_th == "มหาวิทยาลัยมหิดล", FacultyDB.faculty_th == "วิทยาลัยการจัดการ (CMMU)")
            .update({"faculty_th": "วิทยาลัยการจัดการ"}, synchronize_session=False)
        )
        rilca_count = (
            db.query(FacultyDB)
            .filter(FacultyDB.university_th == "มหาวิทยาลัยมหิดล", FacultyDB.faculty_th == "สถาบันวิจัยภาษาและวัฒนธรรมเอเชีย (RILCA)")
            .update({"faculty_th": "สถาบันวิจัยภาษาและวัฒนธรรมเอเชีย"}, synchronize_session=False)
        )
        print(f"  ✅ Aligned {cmmu_count} CMMU records and {rilca_count} RILCA records.")
        db.commit()

        # Step 1: Clean anomalous entries, fix slashes in interests, and publication shape
        all_wave_records = db.query(FacultyDB).filter(
            FacultyDB.id.like("kku_vet%") | FacultyDB.id.like("ku_flas%") | FacultyDB.id.like("kmutnb_fte%")
        ).all()
    ...
    print("\n🎉 Wave 64 duplicate resolution completed successfully!")


if __name__ == "__main__":
    resolve_wave64_duplicates()
