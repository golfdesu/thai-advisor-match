...
# Complete compound title regex with longest match first (Section 9 Invariant 1)
RE_FULL_TITLE = re.compile(
...
for f in faculties:
    ...
    m = RE_FULL_TITLE.match(name)
    if m:
        extracted = m.group(1).strip()
        # Clean title comparison
        clean_title = re.sub(r"\s+", " ", title)
        clean_ext = re.sub(r"\s+", " ", extracted)
        if clean_title in ["นาย", "นาง", "นางสาว"] or (clean_title != clean_ext and not (clean_title.replace(" ", "") == clean_ext.replace(" ", ""))):
            real_mismatches.append((f.id, f.university_th, name, title, extracted))

print(f"Total real title mismatches: {len(real_mismatches)}")
for m in real_mismatches[:30]:
