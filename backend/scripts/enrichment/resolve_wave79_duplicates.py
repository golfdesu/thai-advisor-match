# -*- coding: utf-8 -*-
"""
Resolve Wave 79 Duplicates & Grounding Merge
============================================
Merges duplicates and consolidates profiles following Wave 79 acquisition:
1. Harmonizing Assumption University Courses Naming:
   - Ensuring courses in 'courses' table have exact faculty_th aligned with the 10 academic schools:
     * คณะบริหารธุรกิจและเศรษฐศาสตร์ (MSME) -> คณะบริหารธุรกิจและเศรษฐศาสตร์
     * คณะวิทยาศาสตร์และเทคโนโลยี (VMS) -> คณะวิศวกรรมศาสตร์และวิทยาศาสตร์เทคโนโลยี
     * คณะนิเทศศาสตร์ (Albert Laurence) -> คณะนิเทศศาสตร์
     * บัณฑิตวิทยาลัยบริหารธุรกิจ -> คณะบริหารธุรกิจและเศรษฐศาสตร์
     * บัณฑิตวิทยาลัยบริหารธุรกิจและเทคโนโลยีขั้นสูง -> คณะบริหารธุรกิจและเศรษฐศาสตร์
     * บัณฑิตวิทยาลัยวิทยาศาสตร์และเทคโนโลยี -> คณะวิศวกรรมศาสตร์และวิทยาศาสตร์เทคโนโลยี
     * บัณฑิตวิทยาลัยเทคโนโลยีและการจัดการขั้นสูง -> คณะบริหารธุรกิจและเศรษฐศาสตร์
     * โครงการ AU SIMBA -> คณะบริหารธุรกิจและเศรษฐศาสตร์
2. Synchronizing English university name (university) from TH_TO_EN_CANONICAL ("Assumption University").
...
def resolve_wave79_duplicates():
    print("=================================================================", flush=True)
    print("🔄 RESOLVING WAVE 79 DUPLICATES & PROFILE MERGING", flush=True)
    print("=================================================================", flush=True)

    db = SessionLocal()
    try:
        # Step 0: Harmonize Course-to-Faculty Naming for Assumption University
        print("\n--- Harmonizing Assumption University Courses Naming ---", flush=True)
        univ_name = "มหาวิทยาลัยอัสสัมชัญ"

        course_mappings = [
            (univ_name, "คณะบริหารธุรกิจและเศรษฐศาสตร์ (MSME)", "คณะบริหารธุรกิจและเศรษฐศาสตร์"),
            (univ_name, "คณะวิทยาศาสตร์และเทคโนโลยี (VMS)", "คณะวิศวกรรมศาสตร์และวิทยาศาสตร์เทคโนโลยี"),
            (univ_name, "คณะนิเทศศาสตร์ (Albert Laurence)", "คณะนิเทศศาสตร์"),
            (univ_name, "บัณฑิตวิทยาลัยบริหารธุรกิจ", "คณะบริหารธุรกิจและเศรษฐศาสตร์"),
            (univ_name, "บัณฑิตวิทยาลัยบริหารธุรกิจและเทคโนโลยีขั้นสูง", "คณะบริหารธุรกิจและเศรษฐศาสตร์"),
            (univ_name, "บัณฑิตวิทยาลัยวิทยาศาสตร์และเทคโนโลยี", "คณะวิศวกรรมศาสตร์และวิทยาศาสตร์เทคโนโลยี"),
            (univ_name, "บัณฑิตวิทยาลัยเทคโนโลยีและการจัดการขั้นสูง", "คณะบริหารธุรกิจและเศรษฐศาสตร์"),
            (univ_name, "โครงการ AU SIMBA", "คณะบริหารธุรกิจและเศรษฐศาสตร์"),
        ]
        ...
        # Step 2: Clean anomalous entries, fix slashes in interests, and publication shape for Wave 79
        all_au_records = db.query(FacultyDB).filter(FacultyDB.id.like("au_%")).all()

        nav_boilerplate = ["วิจัย/บริการวิชาการ", "งานวิจัย และงานวิชาการ", "โทรศัพท์", "ติดต่อ", "กยศ.", "ทุนการศึกษา", "ค่าเทอม", "Office:", "Email:"]

        for f in all_au_records:
        ...
        # Email hygiene & trailing junk removal for Wave 79
        for f in all_au_records:
            if f.email:
                em = f.email.strip().lower()
                # Clean trailing concatenated keywords like "office", "mail", "tel"
                m = re.search(r"([a-zA-Z0-9_.+-]+@(?:[a-zA-Z0-9-]+\.)*au\.edu)", em)
                if m:
                    ...
                    m2 = re.search(r"([a-zA-Z0-9_.+-]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,})", em)
                    if m2:
                        clean_em = m2.group(1).strip().lower()
                        if clean_em.endswith("office"):
                            clean_em = clean_em[:-6]
                        if clean_em != f.email:
                            print(f"  [Email Hygiene] Cleaned '{f.email}' -> '{clean_em}'")
                            f.email = clean_em
    ...
    print("\n🎉 Wave 79 duplicate resolution completed successfully!")


if __name__ == "__main__":
    resolve_wave79_duplicates()
