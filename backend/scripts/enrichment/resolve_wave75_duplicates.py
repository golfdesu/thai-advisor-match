# -*- coding: utf-8 -*-
"""
Resolve Wave 75 Duplicates & Grounding Merge
============================================
Merges duplicates and consolidates profiles following Wave 75 acquisition:
1. Harmonizing Mahidol MUIC Courses:
   - 'วิทยาลัยนานาชาติ มหาวิทยาลัยมหิดล' -> 'วิทยาลัยนานาชาติ' (13 courses)
2. Consolidating intra-university duplicate listings:
   - Alessandro Stasi: merge mu_w57_2628_842 into active mu_muic__0110 (MUIC)
   - 14 TU PSDS historical seed duplicates (tu_psds__001..014 into tu_psds__0184..0197)
3. Resolving cross-university institutional transfer:
   - Apiradee Wongkitrungrueng: merge former Chulalongkorn profile (mu_w57_7224_838) into
     active Mahidol MUIC profile (mu_muic__0113), preserving OpenAlex ID A5022335316 and 2,203 citations.
...
6. Archiving merged secondary records to public.scholars_unassigned.
...
10. Enforcing Bibliometric Monotonicity Invariant (total_publications_count >= h_index).
"""
...
def merge_faculty_pair(primary: FacultyDB, secondary: FacultyDB, db) -> str:
    ...
    nav_boilerplate = ["วิจัย/บริการวิชาการ", "งานวิจัย และงานวิชาการ", "วารสารพัฒนศาสตร์", "โทรศัพท์", "ติดต่อ"]
    seen_interests = set()
    cleaned_interests = []
...
def resolve_wave75_duplicates():
    print("=================================================================", flush=True)
    print("🔄 RESOLVING WAVE 75 DUPLICATES & PROFILE MERGING", flush=True)
    print("=================================================================", flush=True)

    db = SessionLocal()
    try:
        # Step 0: Harmonize Course-to-Faculty Naming in courses
        print("\n--- Harmonizing Mahidol MUIC Courses Naming ---", flush=True)
        course_mappings = [
            ("มหาวิทยาลัยมหิดล", "วิทยาลัยนานาชาติ มหาวิทยาลัยมหิดล", "วิทยาลัยนานาชาติ"),
        ]
        ...
        # Step 2: Clean anomalous entries, fix slashes in interests, and publication shape for Wave 75
        wave75_prefixes = ["kku_is%", "mu_muic%", "mu_il%", "tu_psds%"]
        conditions = [FacultyDB.id.like(p) for p in wave75_prefixes]
        ...
        # Step 2b: Targeted Consolidations & Cross-University Institutional Transfers
        print("\n--- Targeted Consolidations & Transfer Merging ---", flush=True)

        # 1. Alessandro Stasi: merge mu_w57_2628_842 into active mu_muic__0110
        p_stasi = db.query(FacultyDB).filter(FacultyDB.id == "mu_muic__0110").first()
        s_stasi = db.query(FacultyDB).filter(FacultyDB.id == "mu_w57_2628_842").first()
        if p_stasi and s_stasi:
            print("  [Merge] Merging mu_w57_2628_842 into active mu_muic__0110 (Alessandro Stasi)")
            merge_faculty_pair(p_stasi, s_stasi, db)
            p_stasi.openalex_id = "A5028042407"
            p_stasi.total_citations = max(p_stasi.total_citations or 0, 607)
            p_stasi.h_index = max(p_stasi.h_index or 0, 12)
            p_stasi.total_publications_count = max(p_stasi.total_publications_count or 0, 12)

        # 2. Apiradee Wongkitrungrueng: merge former Chulalongkorn record into active Mahidol MUIC faculty
        p_api = db.query(FacultyDB).filter(FacultyDB.id == "mu_muic__0113").first()
        s_api = db.query(FacultyDB).filter(FacultyDB.id == "mu_w57_7224_838").first()
        if p_api and s_api:
            print("  [Transfer Merge] Merging former Chula record (mu_w57_7224_838) into active Mahidol MUIC faculty (mu_muic__0113) (Apiradee Wongkitrungrueng)")
            merge_faculty_pair(p_api, s_api, db)
            p_api.openalex_id = "A5022335316"
            p_api.total_citations = max(p_api.total_citations or 0, 2203)
            p_api.h_index = max(p_api.h_index or 0, 7)
            p_api.total_publications_count = max(p_api.total_publications_count or 0, 7)

        # 3. 14 TU PSDS Historical Seeds: merge tu_psds__001..014 into tu_psds__0184..0197
        psds_pairs = [
            ("tu_psds__0184", "tu_psds__009"),  # เกศกุล สระกวี
            ("tu_psds__0185", "tu_psds__006"),  # กฤษณะ โชติรัตนกมล
            ("tu_psds__0186", "tu_psds__002"),  # กนกวรา พวงประยงค์
            ("tu_psds__0187", "tu_psds__008"),  # ณัฐวิคม พันธุวงศ์ภักดี
            ("tu_psds__0188", "tu_psds__007"),  # ณัฐธเดชน์ ชุ่มปลั่ง
            ("tu_psds__0189", "tu_psds__003"),  # ศิญาณี หิรัญสาลี
            ("tu_psds__0190", "tu_psds__004"),  # สานิตย์ หนูนิล
            ("tu_psds__0191", "tu_psds__005"),  # สุภาวดี ขุนทองจันทร์
            ("tu_psds__0192", "tu_psds__001"),  # วีรบูรณ์ วิสารทสกุล
            ("tu_psds__0193", "tu_psds__011"),  # สามชาย ศรีสันต์
            ("tu_psds__0194", "tu_psds__012"),  # ธิดารัตน์ ศักดิ์วีระกุล
            ("tu_psds__0195", "tu_psds__010"),  # กิตติกาญจน์ หาญกุล
            ("tu_psds__0196", "tu_psds__014"),  # อัคคเดช ชัยชนะวิชชกิจ
            ("tu_psds__0197", "tu_psds__013"),  # วรวุฒิ โรมรัตนพันธ์
        ]
        for prim_id, sec_id in psds_pairs:
            prim = db.query(FacultyDB).filter(FacultyDB.id == prim_id).first()
            sec = db.query(FacultyDB).filter(FacultyDB.id == sec_id).first()
            if prim and sec:
                print(f"  [Merge TU PSDS] Merging {sec.id} ({sec.full_name_th}) into active {prim.id}")
                merge_faculty_pair(prim, sec, db)
        ...
        # Email hygiene & trailing junk removal for Wave 75
        for f in all_wave_records:
            if f.email:
        ...
        for oaid, cnt in dup_oa:
            ...
            for secondary in records[1:]:
                ...
                same_person = (
                    (p_cname and s_cname and (p_cname in s_cname or s_cname in p_cname)) or
                    (primary.first_name and secondary.first_name and primary.first_name.lower() == secondary.first_name.lower())
                )
                if same_person:
    ...
    print("\n🎉 Wave 75 duplicate resolution completed successfully!")


if __name__ == "__main__":
    resolve_wave75_duplicates()
