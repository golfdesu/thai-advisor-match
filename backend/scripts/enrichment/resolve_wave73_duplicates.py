# -*- coding: utf-8 -*-
"""
Resolve Wave 73 Duplicates & Grounding Merge
============================================
Merges duplicates and consolidates profiles following Wave 73 acquisition:
1. Intra-university duplicate listings (KKU Fine Arts, KKU MBA, Mahidol Environment, CMU SPP).
2. Harmonizing CMU School of Public Policy in courses & faculties tables:
   'สถาบันนโยบายสาธารณะ' -> 'วิทยาลัยนโยบายสาธารณะ'.
...
11. Enforcing Bibliometric Monotonicity Invariant (total_publications_count >= h_index).
"""
...
def resolve_wave73_duplicates():
    print("=================================================================", flush=True)
    print("🔄 RESOLVING WAVE 73 DUPLICATES & PROFILE MERGING", flush=True)
    print("=================================================================", flush=True)

    db = SessionLocal()
    try:
        # Step 0: Harmonize Course-to-Faculty Naming in courses & faculties
        print("\n--- Harmonizing CMU School of Public Policy Naming ---", flush=True)
        course_mappings = [
            ("มหาวิทยาลัยเชียงใหม่", "สถาบันนโยบายสาธารณะ", "วิทยาลัยนโยบายสาธารณะ"),
        ]
        ...
        # Also harmonize in FacultyDB for CMU SPP
        spp_facs = db.query(FacultyDB).filter(
            FacultyDB.university_th == "มหาวิทยาลัยเชียงใหม่",
            FacultyDB.faculty_th == "สถาบันนโยบายสาธารณะ"
        ).all()
        for f in spp_facs:
            f.faculty_th = "วิทยาลัยนโยบายสาธารณะ"
        if spp_facs:
            print(f"  [Faculty Harmonization] Harmonized {len(spp_facs)} CMU Public Policy faculty records to standard name.")
        db.commit()
        ...
        # Step 2: Clean anomalous entries, fix slashes in interests, and publication shape for Wave 73
        wave73_prefixes = ["kku_fa%", "kku_mba%", "mu_env%", "cmu_spp%"]
        conditions = [FacultyDB.id.like(p) for p in wave73_prefixes]
        from sqlalchemy import or_
        all_wave_records = db.query(FacultyDB).filter(or_(*conditions)).all()

        for f in all_wave_records:
            # Fix double title prefixes
            for prefix in ["ศ.เกียรติคุณ นพ.", "ศ.เกียรติคุณ", "ศ.ดร.", "รศ.ดร.", "ผศ.ดร.", "อ.ดร.", "ศ.", "รศ.", "ผศ.", "อ.", "ดร."]:
                dbl = f"{prefix} {prefix}"
        ...
        # Step 2b: Title prefix repairs
        suthee = db.query(FacultyDB).filter(FacultyDB.id == "cu_cbs_wave11_0413").first()
        if suthee and suthee.full_name_th == "ดร. อ. สุธี โมกขะเวส":
            suthee.academic_title_th = "อ.ดร."
            suthee.full_name_th = "อ.ดร. สุธี โมกขะเวส"

        # Step 2c: Cross-University Institutional Transfer & Historical Seed Consolidations
        print("\n--- Cross-University Institutional Transfer Consolidations ---", flush=True)
        # 1. Nuanchan Singkran (KU -> Mahidol)
        p_nuan = db.query(FacultyDB).filter(FacultyDB.id == "mu_env__010").first()
        s_nuan = db.query(FacultyDB).filter(FacultyDB.id == "ku_env_nuanchan_001").first()
        if p_nuan and s_nuan:
            print("  [Transfer Merge] Merging KU record (ku_env_nuanchan_001) into active Mahidol faculty (mu_env__010)")
            merge_faculty_pair(p_nuan, s_nuan, db)
            p_nuan.openalex_id = "A5008926372"
            p_nuan.total_citations = max(p_nuan.total_citations or 0, 348)
            p_nuan.h_index = max(p_nuan.h_index or 0, 9)
            p_nuan.total_publications_count = max(p_nuan.total_publications_count or 0, 30)

        # 2. Achara Ussawarujikulchai (Chula seed -> Mahidol)
        p_atch = db.query(FacultyDB).filter(FacultyDB.id == "mu_env__033").first()
        s_atch = db.query(FacultyDB).filter(FacultyDB.id == "cu_eng_atchara_001").first()
        if p_atch and s_atch:
            print("  [Transfer Merge] Merging CU seed record (cu_eng_atchara_001) into active Mahidol faculty (mu_env__033)")
            merge_faculty_pair(p_atch, s_atch, db)
            p_atch.openalex_id = "A5048103814"
        ...
        # Email hygiene & trailing junk removal for Wave 73
        for f in all_wave_records:
            if f.email:
        ...
        for oaid, cnt in dup_oa:
            ...
            for secondary in records[1:]:
                ...
                # Verify if this is actually the same individual
                same_person = (
    ...
    print("\n🎉 Wave 73 duplicate resolution completed successfully!")


if __name__ == "__main__":
    resolve_wave73_duplicates()
