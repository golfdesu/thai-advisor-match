...
from app.core.university_canonicalizer import get_university_dedup_key, canonicalize_university_th
...
def deep_merge_and_deduplicate():
    ...
    try:
        ...
        for f in faculties:
            ...
            # Also group by (canonical university_th, normalized thai name) or (canonical university key, first_name, last_name)
            if f.university_th and f.full_name_th:
                clean_th_name = _strip_leading_title_tokens(f.full_name_th.strip())
                if clean_th_name:
                    uni_key = canonicalize_university_th(f.university_th)
                    name_groups[(uni_key, clean_th_name)].append(f)
            elif f.university and f.first_name and f.last_name:
                uni_key = get_university_dedup_key(f.university)
                name_groups[(uni_key, f.first_name.strip().lower(), f.last_name.strip().lower())].append(f)
