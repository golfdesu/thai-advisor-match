# -*- coding: utf-8 -*-
"""
Resolve Wave 72 Duplicates & Grounding Merge
============================================
Merges duplicates and consolidates profiles following Wave 72 acquisition:
1. Intra-university duplicate listings (Silpakorn Fine Arts, KKU PH, KMUTT JGSEE, Silpakorn Music).
2. Harmonizing KMUTT JGSEE graduate courses in courses & faculties tables:
   'บัณฑิตวิทยาลัยร่วมด้านพลังงานและสิ่งแวดล้อม (JGSEE)' -> 'บัณฑิตวิทยาลัยร่วมด้านพลังงานและสิ่งแวดล้อม'.
...
def resolve_wave72_duplicates():
    print("=================================================================", flush=True)
    print("🔄 RESOLVING WAVE 72 DUPLICATES & PROFILE MERGING", flush=True)
    print("=================================================================", flush=True)

    db = SessionLocal()
    try:
        # Step 0: Harmonize Course-to-Faculty Naming in courses & faculties
        print("\n--- Harmonizing JGSEE Faculty Naming ---", flush=True)
        course_mappings = [
            ("มหาวิทยาลัยเทคโนโลยีพระจอมเกล้าธนบุรี", "บัณฑิตวิทยาลัยร่วมด้านพลังงานและสิ่งแวดล้อม (JGSEE)", "บัณฑิตวิทยาลัยร่วมด้านพลังงานและสิ่งแวดล้อม"),
        ]
        ...
        # Also harmonize in FacultyDB for KMUTT JGSEE
        jgsee_facs = db.query(FacultyDB).filter(
            FacultyDB.university_th == "มหาวิทยาลัยเทคโนโลยีพระจอมเกล้าธนบุรี",
            FacultyDB.faculty_th == "บัณฑิตวิทยาลัยร่วมด้านพลังงานและสิ่งแวดล้อม (JGSEE)"
        ).all()
        for f in jgsee_facs:
            f.faculty_th = "บัณฑิตวิทยาลัยร่วมด้านพลังงานและสิ่งแวดล้อม"
        if jgsee_facs:
            print(f"  [Faculty Harmonization] Harmonized {len(jgsee_facs)} JGSEE faculty records to standard name.")
        db.commit()
        ...
        # Step 2: Clean anomalous entries, fix slashes in interests, and publication shape for Wave 72
        wave72_prefixes = ["su_psg%", "kku_ph%", "kmutt_jgsee%", "su_mus%"]
        conditions = [FacultyDB.id.like(p) for p in wave72_prefixes]
        ...
        # Email hygiene & trailing junk removal for Wave 72
        for f in all_wave_records:
            if f.email:
    ...
    print("\n🎉 Wave 72 duplicate resolution completed successfully!")


if __name__ == "__main__":
    resolve_wave72_duplicates()
