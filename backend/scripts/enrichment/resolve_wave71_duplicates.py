# -*- coding: utf-8 -*-
"""
Resolve Wave 71 Duplicates & Grounding Merge
============================================
Merges duplicates and consolidates profiles following Wave 71 acquisition:
1. Intra-university duplicate listings (CMU Fine Arts, CMU ICDI, Mahidol CMMU, KMITL AMI).
2. Harmonizing Kasetsart University FLAS graduate courses in courses table:
   'คณะศิลปศาสตร์และวิทยาศาสตร์' -> 'คณะศิลปศาสตร์และวิทยาศาสตร์ กำแพงแสน'.
...
def resolve_wave71_duplicates():
    print("=================================================================", flush=True)
    print("🔄 RESOLVING WAVE 71 DUPLICATES & PROFILE MERGING", flush=True)
    print("=================================================================", flush=True)

    db = SessionLocal()
    try:
        # Step 0: Harmonize Course-to-Faculty Naming in courses
        print("\n--- Harmonizing Course Faculty Naming ---", flush=True)
        course_mappings = [
            ("มหาวิทยาลัยเกษตรศาสตร์", "คณะศิลปศาสตร์และวิทยาศาสตร์", "คณะศิลปศาสตร์และวิทยาศาสตร์ กำแพงแสน"),
            ("มหาวิทยาลัยเชียงใหม่", "วิทยาลัยนวัตกรรมดิจิทัล (นานาชาติ)", "วิทยาลัยนานาชาตินวัตกรรมดิจิทัล"),
        ]
        ...
        # Step 2: Clean anomalous entries, fix slashes in interests, and publication shape for Wave 71
        wave71_prefixes = ["cmu_fa%", "cmu_icdi%", "mu_cmmu%", "kmitl_ami%"]
        conditions = [FacultyDB.id.like(p) for p in wave71_prefixes]
        from sqlalchemy import or_
        all_wave_records = db.query(FacultyDB).filter(or_(*conditions)).all()

        for f in all_wave_records:
            # Fix double title prefixes
            for prefix in ["ศ.เกียรติคุณ นพ.", "ศ.ดร.", "รศ.ดร.", "ผศ.ดร.", "อ.ดร.", "ศ.", "รศ.", "ผศ.", "อ.", "ดร."]:
                dbl = f"{prefix} {prefix}"
        ...
        transfer_pairs = [
            ("cmu_fa__005", "silpakornu_facultyoff_sudhom_008"),
            ("cmu_fa__018", "silpakornu_facultyofa_panjinda_013"),
            ("cmu_fa__019", "silpakornu_facultyofa_maleepan_014"),
            ("cmu_fa__039", "silpakornu_facultyoff_saenyakiatkun_010"),
        ]
        for p_id, s_id in transfer_pairs:
            p_rec = db.query(FacultyDB).filter(FacultyDB.id == p_id).first()
            s_rec = db.query(FacultyDB).filter(FacultyDB.id == s_id).first()
            if p_rec and s_rec:
                print(f"  [Transfer Merge] Merging SU record ({s_rec.id}) into active CMU faculty ({p_rec.id})")
    ...
    print("\n🎉 Wave 71 duplicate resolution completed successfully!")


if __name__ == "__main__":
    resolve_wave71_duplicates()
