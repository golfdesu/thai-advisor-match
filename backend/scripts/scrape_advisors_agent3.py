...
logger = logging.getLogger("Advisor_Agent3")

from google import genai
from google.genai import types

UNIVERSITIES = ["Kasetsart University"]
FACULTIES = [
...
def run_scraper():
    ...
    with Session(engine) as session:
        for uni in UNIVERSITIES:
            for fac in FACULTIES:
                logger.info(f"=== Advisor Agent 3 Target: {uni} | {fac} ===")
    ...
    logger.info(f"Advisor Agent 3 completed! Total: {total_scraped}")
